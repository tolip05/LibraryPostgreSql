...
public class DbfReader implements AutoCloseable {
    ...
    public DbfReader(File file) throws DbfException {
        try {
            dataInput = new RandomAccessFile(file,
                    "r");
            header = DbfHeader.read(dataInput);
            skipToDataBeginning();
        } catch (IOException e) {
            throw new DbfException("Cannot open Dbf file "
                    + file,
                    e);
        }
    }
    ...
    public DbfReader(InputStream in) throws DbfException {
        try {
            ...
            throw new DbfException("Cannot read Dbf",
                    e);
        }
    }
    ...
    private void skipToDataBeginning() throws IOException {
        ...
        int dataStartIndex = header.getHeaderLength()
                - 32
                * (header.getFieldsCount()
                + 1)
                - 1;
        if (dataStartIndex > 0) {
    ...
    /**
     * @return {@code true} if the reader can seek forward or backward to a
     *         specified record index, {@code false} otherwise.
     */
    ...
    /**
     * Attempt to seek to a specified record index. If successful the record can
     * be read by calling {@link DbfReader#nextRecord()}.
     *
     * @param n
    ...
    public void seekToRecord(int n) {
        ...
        if (n < 0
                || n >= header.getNumberOfRecords()) {
            throw new DbfException(String.format("Record index out of range [0, %d]: %d",
                    header.getNumberOfRecords(),
                    n));
        }
        long position = header.getHeaderLength()
                + n
                * header.getRecordLength();
        try {
            ((RandomAccessFile) dataInput).seek(position);
        } catch (IOException e) {
            throw new DbfException(String.format("Failed to seek to record %d of %d",
                    n,
                    header.getNumberOfRecords()),
                    e);
        }
        ...
        try {
            ...
            do {
                ...
                if (nextByte == DATA_ENDED) {
                    ...
                    dataInput.skipBytes(header.getRecordLength()
                            - 1);
                }
            ...
            throw new DbfException("Cannot read next record form Dbf file",
                    e);
        }
    }
    ...
    protected Object readCharacterValue(DbfField field,
                                        byte[] buf)
            throws IOException {
        return buf;
    }

    protected Date readDateValue(DbfField field,
                                 byte[] buf)
            throws IOException {
        int year = DbfUtils.parseInt(buf,
                0,
                4);
        int month = DbfUtils.parseInt(buf,
                4,
                6);
        int day = DbfUtils.parseInt(buf,
                6,
                8);
    ...
    protected Float readFloatValue(DbfField field,
                                   byte[] buf)
            throws IOException {
        try {
            byte[] floatBuf = DbfUtils.trimLeftSpaces(buf);
            boolean processable = (floatBuf.length > 0
                    && !DbfUtils.contains(floatBuf,
                    (byte) '?'));
            return processable ? Float.valueOf(new String(floatBuf)) : null;
        } catch (NumberFormatException e) {
            throw new DbfException("Failed to parse Float from "
                    + field.getName(),
                    e);
        }
    }

    protected Boolean readLogicalValue(DbfField field,
                                       byte[] buf)
            throws IOException {
        boolean isTrue = (buf[0] == 'Y'
                || buf[0] == 'y'
                || buf[0] == 'T'
                || buf[0] == 't');
    ...
    protected Number readNumericValue(DbfField field,
                                      byte[] buf)
            throws IOException {
        try {
            byte[] numericBuf = DbfUtils.trimLeftSpaces(buf);
            boolean processable = numericBuf.length > 0
                    && !DbfUtils.contains(numericBuf,
                    (byte) '?');
            return processable ? Double.valueOf(new String(numericBuf)) : null;
        } catch (NumberFormatException e) {
            throw new DbfException("Failed to parse Number from "
                    + field.getName(),
                    e);
        }
    }

    protected Number readMemoLink(DbfField field,
                                  byte[] buf)
            throws IOException {
        switch (field.getFieldLength()) {
            ...
            case 10:
                return readNumericValue(field,
                        buf);
            default:
                throw new DbfException("Unknown MEMO mode: "
